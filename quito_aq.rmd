...
for (i in year){
  for (j in month){
    polar.plot(aq_monthly_by_hour$conc[aq_monthly_by_hour$month == j & 
...
polar.plot(aq_by_hour$conc,
           ...
           radial.lim = c(0, 250), # defaults to min and max
           add = TRUE)
polar.plot(aq_monthly_by_hour$conc[aq_monthly_by_hour$month == "2" & aq_monthly_by_hour$year == "2016"], 
           start = 90, 
           clockwise = T, 
           rp.type = "polygon",
           line.col = "green", 
...
plot(aq_by_month$conc,
  ...
  text(seq(1,73,by=2), par("usr")[3]-0.5, 
       ...
       cex=0.8)
  axis(2, at= seq(0,(max(aq_by_month$conc)+50),by=50),
       tck = 0, las = 1,
       labels = seq(0,(max(aq_by_month$conc)+50),by=50))
  par(xpd = FALSE)
  grid()
   points(x = aq_by_month$yrmonth[aq_by_month$yrmonth =="2016-2"],
        y = min(aq_by_month$conc),
        col = "red",
        pch = 16)
   points(x = aq_by_month$yrmonth[aq_by_month$yrmonth =="2011-1"],
         y = aq_by_month$conc[aq_by_month$yrmonth=="2011-1"],
         col = "blue",
         pch = 16)
   text(x = aq_by_month$yrmonth[aq_by_month$yrmonth =="2015-10"],
       y = min(aq_by_month$conc)-20,
       paste("PM2.5 = ", round(min(aq_by_month$conc),2)," in Feb 2016", sep = "\n"),
       adj = c(0,1),
       offset = c(0,-2))
   text(x = aq_by_month$yrmonth[aq_by_month$yrmonth =="2011-2"],
       y = aq_by_month$conc[aq_by_month$yrmonth=="2011-1"]-20,
       paste("PM2.5 = ", round(aq_by_month$conc[aq_by_month$yrmonth=="2011-1"],2), 
             "in Jan 2011", sep = "\n"),
       adj = c(0,1))
```
